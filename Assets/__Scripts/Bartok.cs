...
public void PassTurn(int num = -1) {
    ...
    public CardBartok MoveToDiscard(CardBartok tCB) {
        tCB.state = CBState.discard; discardPile.Add(tCB);
        tCB.SetSortingLayerName(layout.discardPile.layerName);
        tCB.SetSortOrder(discardPile.Count * 4);
        tCB.transform.localPosition = layout.discardPile.pos + Vector3.back / 2; return (tCB);
    }
    ...
    public	CardBartok	Draw()	{
        ...
        drawPile.RemoveAt(0);
        //	Then	remove	it	from List<>	drawPile		
        return (cd);
        //	And	return	it			
    }
